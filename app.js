...
function connected(jsn) {
    ...
    $SD.on('se.gazab.alertmanager.action.willAppear', (jsonObj) => action.onWillAppear(jsonObj));
    $SD.on('se.gazab.alertmanager.action.keyUp', (jsonObj) => action.onKeyUp(jsonObj));
    $SD.on('se.gazab.alertmanager.action.sendToPlugin', (jsonObj) => action.onSendToPlugin(jsonObj));
    $SD.on('se.gazab.alertmanager.action.didReceiveSettings', (jsonObj) => action.onDidReceiveSettings(jsonObj));
    $SD.on('se.gazab.alertmanager.action.propertyInspectorDidAppear', (jsonObj) => {
        console.log('%c%s', 'color: white; background: black; font-size: 13px;', '[app.js]propertyInspectorDidAppear:');
    });
    $SD.on('se.gazab.alertmanager.action.propertyInspectorDidDisappear', (jsonObj) => {
